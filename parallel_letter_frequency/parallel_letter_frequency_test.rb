...
class ParallelLetterFrequencyTest < Minitest::Test
  ...
  def test_combo_lower_upper_punctuation_whitespace
    # skip
    ruby_wiki = File.read(File.expand_path('ruby_wiki.txt', __dir__))
    texts = [ruby_wiki]
    expected = {
      "r" => 34,
      "u" => 20,
  ...
  def test_large_texts
    # skip
    ch37 = File.read(File.expand_path('moby_dick_ch37.txt', __dir__))
    ch38 = File.read(File.expand_path('moby_dick_ch38.txt', __dir__))
    ch39 = File.read(File.expand_path('moby_dick_ch39.txt', __dir__))
